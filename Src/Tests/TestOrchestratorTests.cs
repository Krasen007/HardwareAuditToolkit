...
public class TestOrchestratorTests
{
    ...
    {
        using var orchestrator = new TestOrchestrator(new AuditSession(), Array.Empty<ITestModule>());
    ...
    public void ExclusiveModule_BlocksAnotherExclusiveModule()
    {
        ...
        using var orchestrator = new TestOrchestrator(session, new[] { keyboard, mouse });
    ...
    public void NonExclusiveModule_CanRunAlongsideExclusiveModule()
    {
        ...
        using var orchestrator = new TestOrchestrator(session, new[] { keyboard, sensors });
    ...
    public void ExclusiveModule_CanStartWhileNonExclusiveModuleRuns()
    {
        ...
        var keyboard = new FakeModule("keyboard", isExclusive: true);
        using var orchestrator = new TestOrchestrator(session, new[] { sensors, keyboard });
    ...
    public void CancelModule_RecordsCancelledStatus()
    {
        var session = new AuditSession();
        var keyboard = new FakeModule("keyboard", isExclusive: true);
        using var orchestrator = new TestOrchestrator(session, new[] { keyboard });
    ...
    private sealed class FakeModule : ITestModule
    {
        private readonly TestStatus? _autoCompleteStatus;

        public FakeModule(string id, bool isExclusive, TimeSpan? maxDuration = null, TestStatus? autoComplete = null)
        {
            ModuleId = id;
            _autoCompleteStatus = autoComplete;
            Metadata = new FakeMetadata(id, isExclusive, maxDuration);
        }
        ...
        {
            public FakeMetadata(string id, bool isExclusive, TimeSpan? maxDuration)
            {
                Id = id;
